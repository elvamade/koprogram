    ...
    IDisposable, IHostedService
{
        ...
        where TEventData : class
    {
        if (eventData == null)
        {
            logger.LogWarning("Skip publishing null event data for {EventType}", typeof(TEventData).Name);
            return;
        }

        if (_rabbitMQConnection == null || !_rabbitMQConnection.IsOpen)
        {
            logger.LogWarning("Skip publishing {EventType}: RabbitMQ connection is not open", eventType ?? typeof(TEventData).Name);
            return;
        }
