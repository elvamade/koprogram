...
public static class MyTelegramServerReadModelMongoDbExtensions
{
    public static void RegisterMongoDbSerializer(this IServiceCollection services)
    {
        ...
        // Register class map for base StarGiftReadModel to ignore its Id property
        // This prevents conflict with the derived MongoDB.StarGiftReadModel's Id mapping
        if (!BsonClassMap.IsClassMapRegistered(typeof(Impl.StarGiftReadModel)))
        {
            BsonClassMap.RegisterClassMap<Impl.StarGiftReadModel>(cm =>
            {
                cm.AutoMap();
    ...
    public static IEventFlowOptions AddMyTelegramMongoDbReadModel(this IEventFlowOptions options)
    {
        ...
        return options.AddDefaults(typeof(MyTelegramServerReadModelMongoDbExtensions).Assembly)
            ...
            // Query-only read models (populated externally, not via EventFlow)
            .UseMongoDbQueryOnlyReadModel<StarGiftReadModel>()
            ;
    }
}
