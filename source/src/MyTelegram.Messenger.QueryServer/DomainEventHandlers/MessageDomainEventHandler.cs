        ...
        ISubscribeSynchronousTo<SendMessageSaga, SendMessageSagaId, ReceiveInboxMessageCompletedSagaEvent>
{
    ...
    private async Task HandleSendMessageAsync(SendOutboxMessageCompletedSagaEvent aggregateEvent)
    {
        ...
        // For StarGiftPurchase, the handler already returns TPaymentResult with updates,
        // so we only need to send updates to sender's other devices (not the requesting device)
        if (item.MessageSubType == MessageSubType.StarGiftPurchase)
        {
            // Only send to other devices, excluding the requesting device
    ...
    private async Task HandleSendMessageToChannelAsync(SendOutboxMessageCompletedSagaEvent aggregateEvent)
    {
        ...
        else if (aggregateEvent.RequestInfo.ReqMsgId == 0 /*|| item.MessageSubType == MessageSubType.ForwardMessage*/)
        {
