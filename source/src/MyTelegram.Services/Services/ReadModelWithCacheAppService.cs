    ...
    where TReadModel : IReadModel
{
    public Task<TReadModel?> GetAsync(long? id)
    {
        if (!id.HasValue || id == 0)
        {
            return Task.FromResult<TReadModel?>(default);
        }

        return GetOrRefreshAsync(id.Value);
    }

    public async Task<TReadModel> GetAsync(long id)
    {
        var readModel = await GetOrRefreshAsync(id);
    ...
    private async Task<TReadModel?> GetOrRefreshAsync(long id)
    {
        var readModel = await cacheHelper.GetOrCreateAsync(id, () => GetReadModelAsync(id), GetReadModelId);
        if (readModel == null || IsValidReadModel(readModel))
        {
            return readModel;
        }

        var refreshed = await GetReadModelAsync(id);
        if (refreshed != null && IsValidReadModel(refreshed))
        {
            cacheHelper.Add(id, GetReadModelId(refreshed), refreshed);
            return refreshed;
        }
