    ...
    ILayeredService<IPhotoConverter> photoLayeredService) : IUserConverterService, ITransientDependency
{
    ...
    private const string FrozenAboutText = "Аккаунт заморожен";
    private const string ScamAboutText =
        "⚠️ Внимание: на этот аккаунт много жаловались в связи с мошенничеством или попытками выдать себя за официальный аккаунт. Будьте осторожны, особенно если у Вас попросят денег.";
        ...
        IReadOnlyCollection<IPrivacyReadModel>? privacyReadModels, int layer = 0)
    {
        ...
        {
            userFull.About = AppendAboutWarning(userFull.About, FrozenAboutText);
        }

        var isScamUser = ScamUserIds.Contains(userReadModel.UserId);
        if (isScamUser)
        {
            userFull.About = AppendAboutWarning(userFull.About, ScamAboutText);
        }
        ...
        IReadOnlyCollection<IPrivacyReadModel>? privacyReadModels = null, int layer = 0)
    {
        ...
        if (userReadModel.EmojiStatusDocumentId != null)
        {
            emojiStatus = new EmojiStatus(
                ...
                userReadModel.EmojiStatusCollectibleTitle,
                userReadModel.EmojiStatusCollectibleSlug,
                userReadModel.EmojiStatusCollectiblePatternDocumentId,
                userReadModel.EmojiStatusCollectibleCenterColor,
                userReadModel.EmojiStatusCollectibleEdgeColor,
                userReadModel.EmojiStatusCollectiblePatternColor,
                userReadModel.EmojiStatusCollectibleTextColor
            );
        }
    ...
    private static string AppendAboutWarning(string? about, string warningText)
    {
        ...
        if (about.Contains(warningText, StringComparison.Ordinal))
        {
            return about;
        }

        return $"{about} | {warningText}";
    }
}
