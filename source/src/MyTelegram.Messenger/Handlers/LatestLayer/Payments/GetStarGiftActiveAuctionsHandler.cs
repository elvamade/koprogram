...
internal sealed class GetStarGiftActiveAuctionsHandler(
    ...
    : RpcResultObjectHandler<MyTelegram.Schema.Payments.RequestGetStarGiftActiveAuctions, MyTelegram.Schema.Payments.IStarGiftActiveAuctions>, IObjectHandler
{
    protected override async Task<MyTelegram.Schema.Payments.IStarGiftActiveAuctions> HandleCoreAsync(
        IRequestInput input,
        MyTelegram.Schema.Payments.RequestGetStarGiftActiveAuctions obj)
    {
        var now = (int)DateTimeOffset.UtcNow.ToUnixTimeSeconds();
        var giftsCollection = mongoDatabase.GetCollection<BsonDocument>("eventflow-stargiftreadmodel");

        // Round settlement is triggered by client polling if no new bids are made.
        var dueGiftIds = await giftsCollection.Find(
                Builders<BsonDocument>.Filter.And(
                    Builders<BsonDocument>.Filter.Eq("Auction", true),
                    Builders<BsonDocument>.Filter.Lte("NextRoundAt", now)))
            .Project(Builders<BsonDocument>.Projection.Include("GiftId"))
            .Limit(100)
            .ToListAsync();
        foreach (var dueGift in dueGiftIds)
        {
            var giftId = dueGift["GiftId"].IsInt64 ? dueGift["GiftId"].AsInt64 : dueGift["GiftId"].AsInt32;
            await StarGiftAuctionRoundProcessor.ProcessDueRoundsAsync(mongoDatabase, idGenerator, giftId, now);
        }

        // Get active auction gifts from database
        var filter = Builders<BsonDocument>.Filter.And(
            Builders<BsonDocument>.Filter.Eq("Auction", true),
            Builders<BsonDocument>.Filter.Or(
                Builders<BsonDocument>.Filter.Gt("AuctionEndDate", now),
                Builders<BsonDocument>.Filter.Exists("AuctionEndDate", false)
            )
        );

        var giftDocs = await giftsCollection.Find(filter).ToListAsync();

        if (giftDocs.Count == 0)
        {
            // Check hash for not modified response
            if (obj.Hash != 0)
            {
        ...
        var documentMap = docDocs.ToDictionary(d => GetLong(d, "DocumentId"));

        var auctions = new TVector<MyTelegram.Schema.IStarGiftActiveAuctionState>();
        var userIds = new HashSet<long>();

        foreach (var g in giftDocs)
        {
            var stickerId = GetLong(g, "StickerId");
            IDocument sticker = documentMap.TryGetValue(stickerId, out var docBson)
                ? ConvertDocument(docBson)
                : new TDocumentEmpty { Id = stickerId };

            var gift = BuildStarGift(g, sticker);
            var auctionState = BuildAuctionState(g);
            var userState = await BuildUserStateAsync(input.UserId, GetLong(g, "GiftId"));
            CollectAuctionUserIds(userIds, g, auctionState, userState);

            auctions.Add(new TStarGiftActiveAuctionState
            {
                Gift = gift,
                State = auctionState,
                UserState = userState
    ...
    private static long CalculateHash(List<BsonDocument> gifts)
    {
        ...
        unchecked
        {
            ...
            foreach (var g in gifts.OrderBy(x => GetLong(x, "GiftId")))
            {
                ...
                var auctionVersion = GetNullableInt(g, "AuctionVersion") ?? 0;
                var currentRound = GetNullableInt(g, "CurrentRound") ?? 0;
                var nextRoundAt = GetNullableInt(g, "NextRoundAt") ?? 0;
                var minBidAmount = GetNullableLong(g, "MinBidAmount") ?? 0;
                hash = (hash * 31) ^ auctionVersion;
                hash = (hash * 31) ^ currentRound;
                hash = (hash * 31) ^ nextRoundAt;
                hash = (hash * 31) ^ minBidAmount;

                if (g.Contains("TopBidders") && g["TopBidders"].IsBsonArray)
                {
                    foreach (var bidder in g["TopBidders"].AsBsonArray.Take(20))
                    {
                        var bidderId = bidder.IsInt64 ? bidder.AsInt64 : bidder.AsInt32;
                        hash = (hash * 31) ^ bidderId;
                    }
                }
            }
            return hash;
    ...
    private static void CollectAuctionUserIds(
        HashSet<long> userIds,
        BsonDocument giftDoc,
        MyTelegram.Schema.IStarGiftAuctionState auctionState,
        MyTelegram.Schema.IStarGiftAuctionUserState userState)
    {
        ...
        if (userState.BidPeer is TPeerUser userPeer)
        {
