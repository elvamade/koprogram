    ...
    : RpcResultObjectHandler<RequestSendMessage, IUpdates>
{
    protected override async Task<IUpdates> HandleCoreAsync(IRequestInput input, RequestSendMessage obj)
    {
        ...
        await paidMessagesAppService.ChargePaidMessagesAsync(input.UserId, toPeer, obj.AllowPaidStars, 1);
        ...
        var botFatherReply = GetBotFatherReply(obj.Message, toPeer, input.UserId);
        if (!string.IsNullOrEmpty(botFatherReply))
        {
            var botRequestInfo = input.ToRequestInfo() with
            {
                UserId = MyTelegramConsts.BotFatherUserId,
                RequestId = Guid.NewGuid(),
                Date = DateTimeOffset.UtcNow.ToUnixTimeMilliseconds()
            };
            var botReplyInput = new SendMessageInput(
                botRequestInfo,
                MyTelegramConsts.BotFatherUserId,
                input.UserId.ToUserPeer(),
                botFatherReply,
                Random.Shared.NextInt64());
            await messageAppService.SendMessageAsync([botReplyInput]);
        }

        return null !;
    }

    private static string? GetBotFatherReply(string? message, Peer toPeer, long senderUserId)
    {
        if (toPeer.PeerType != PeerType.User || toPeer.PeerId != MyTelegramConsts.BotFatherUserId)
        {
            return null;
        }

        if (senderUserId == MyTelegramConsts.BotFatherUserId)
        {
            return null;
        }
...
Send me a display name for your bot.
Then send me a unique username ending in "bot", for example: my_sample_bot.
""",
            "/mybots" => "Use /mybots to manage your bots. Feature is in progress in this server build.",
            "/token" => "Use /token to regenerate your bot token. Feature is in progress in this server build.",
            "/setdescription" => "Use /setdescription to set your bot description. Feature is in progress in this server build.",
            "/help" => "Use /start to see the command list. Main command for creating a bot: /newbot",
            _ => "Unknown command. Send /start to see available BotFather commands."
        };
    }
    ...
    private static string ExtractCommand(string? message)
    {
        ...
        var firstToken = message.Trim().Split(' ', StringSplitOptions.RemoveEmptyEntries).FirstOrDefault() ?? string.Empty;
