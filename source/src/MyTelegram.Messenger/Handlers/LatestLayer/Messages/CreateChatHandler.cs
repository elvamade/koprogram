...
/// </remarks>
internal sealed class CreateChatHandler(ICommandBus commandBus, IIdGenerator idGenerator, IRandomHelper randomHelper, IAccessHashHelper accessHashHelper, IQueryProcessor queryProcessor, IPeerHelper peerHelper, IPrivacyAppService privacyAppService, IActionRateLimitService actionRateLimitService) : RpcResultObjectHandler<Schema.Messages.RequestCreateChat, Schema.Messages.IInvitedUsers>
{
    protected override async Task<Schema.Messages.IInvitedUsers> HandleCoreAsync(IRequestInput input, RequestCreateChat obj)
    {
        var createdChatCount = await queryProcessor.ProcessAsync(new GetCreatedChannelCountByCreatorQuery(input.UserId, false));
        if (createdChatCount >= MyTelegramConsts.MaxCreatedChatCount)
        {
            RpcErrors.RpcErrors400.ChannelsTooMuch.ThrowRpcError();
        }
        ...
        {
            var contactUserIdList = await queryProcessor.ProcessAsync(new GetContactUserIdListByTargetUserIdListQuery(input.UserId, memberUserIds));
            var nonContactInviteCount = memberUserIds.Count - contactUserIdList.Count;
            if (nonContactInviteCount > 0)
            {
                var waitSeconds = await actionRateLimitService.CheckAndIncrementAsync($"invite:non-contact:{input.UserId}", MyTelegramConsts.NonContactInviteLimitPerDay, 86400, nonContactInviteCount);
                if (waitSeconds > 0)
                {
