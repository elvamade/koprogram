...
internal sealed class GetPeerSettingsHandler(
    ...
    : RpcResultObjectHandler<MyTelegram.Schema.Messages.RequestGetPeerSettings, MyTelegram.Schema.Messages.IPeerSettings>
{
    protected override async Task<MyTelegram.Schema.Messages.IPeerSettings> HandleCoreAsync(IRequestInput input, RequestGetPeerSettings obj)
    {
        ...
        if (peer.PeerId == MyTelegramConsts.OfficialUserId || peer.PeerType == PeerType.Self || peer.PeerId == input.UserId)
        {
        ...
        var requiredPaidStars = await paidMessagesAppService.GetRequiredPaidStarsAsync(input.UserId, peer);
        settings.ChargePaidMessageStars = requiredPaidStars > 0 ? requiredPaidStars : null;
