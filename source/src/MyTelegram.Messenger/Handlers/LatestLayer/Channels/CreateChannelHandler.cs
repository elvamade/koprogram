...
/// </remarks>
internal sealed class CreateChannelHandler(ICommandBus commandBus, IIdGenerator idGenerator, IRandomHelper randomHelper, IQueryProcessor queryProcessor) : RpcResultObjectHandler<MyTelegram.Schema.Channels.RequestCreateChannel, MyTelegram.Schema.IUpdates>
{
    protected override async Task<IUpdates> HandleCoreAsync(IRequestInput input, RequestCreateChannel obj)
    {
        var maxChannelCount = obj.Broadcast
            ? MyTelegramConsts.MaxCreatedBroadcastChannelCount
            : MyTelegramConsts.MaxCreatedChatCount;
        var createdChannelCount = await queryProcessor.ProcessAsync(new GetCreatedChannelCountByCreatorQuery(input.UserId, obj.Broadcast));
        if (createdChannelCount >= maxChannelCount)
        {
